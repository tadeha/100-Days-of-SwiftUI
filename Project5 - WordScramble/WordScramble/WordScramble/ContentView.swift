...
//  WordScramble
//
//  Created by Tadeh Alexani on 4/15/20.
...
/*
 ...
 return Text("\(allGood ? "All Good" : "It's a misspelling!")")
...
struct ContentView: View {
  ...
  func addNewWord() {
    ...
    guard !isSame(word: answer) else {
      wordError(title: "Words are same", message: "Be more creative!")
      return
    }
  ...
  func startGame() {
    ...
    if let startTextUrl = Bundle.main.url(forResource: "start", withExtension: "txt") {
      if let startTextString = try? String(contentsOf: startTextUrl) {
        let allWords = startTextString.components(separatedBy: "\n")
